...
class CountryScreen extends StatefulWidget {
  static String route = '/admin/state';
...
class CountryScreenState extends State<CountryScreen> {
  ...
  getCountryListApiCall() async {
    ...
    await getCountryList(page: currentPage, isDeleted: true, perPage: perPage)
        .then((value) {
      ...
      stateList.clear();
      stateList.addAll(value.data!);
      if (currentPage != 1 && stateList.isEmpty) {
        currentPage -= 1;
        getCountryListApiCall();
      }
      setState(() {});
  ...
  restoreCountryApiCall({@required int? id, @required String? type}) async {
    ...
    appStore.setLoading(true);
    await stateAction(req).then((value) {
      appStore.setLoading(false);
      getCountryListApiCall();
      toast(value.message.toString());
    }).catchError((error) {
  ...
  updateStatusApiCall(CountryData stateData) async {
    Map req = {
      "id": stateData.id,
  ...
  Widget build(BuildContext context) {
    Widget addCountryButton() {
      return AddButtonComponent(language.add_state, () {
        showDialog(
          context: context,
                                  ...
                                  child: ConstrainedBox(
                                    ...
                                    child: DataTable(
                                      ...
                                      columns: [
                                        ...
                                        DataColumn(
                                            label: Text(language.state_name)),
                                        DataColumn(
                                            label:
                                                Text(language.distance_type)),
                                        DataColumn(
                                            label: Text(language.weight_type)),
                                        DataColumn(
                                            label: Text(language.created_date)),
                                        DataColumn(
                                            label: Text(language.status)),
                                        DataColumn(
                                            label: Text(language.actions)),
                                      ],
                                      rows: stateList.map((CountryData mData) {
                                                  ...
                                                  ? commonConfirmationDialog(
                                                      ...
                                                      subtitle: mData.status !=
                                                          ...
                                                          : language
                                                              .disable_state_msg)
                                                  : toast(language
                                                      .you_cannot_update_status_record_deleted);
                                            },
                                          )),
                                          DataCell(
                                            Row(
                                              children: [
                                                outlineActionIcon(
                                                    mData.deletedAt == null
                                                        ...
                                                        : Icons.restore,
                                                    Colors.green,
                                                    '${mData.deletedAt == null ? language.edit : language.restore}',
                                                    () {
                                                  mData.deletedAt == null
                                                      ? showDialog(
                                                          context: context,
                                                      ...
                                                      : commonConfirmationDialog(
                                                          ...
                                                          title: language
                                                              .restore_state,
                                                          subtitle: language
                                                              .do_you_want_to_restore_this_state);
                                                }),
                                                SizedBox(width: 8),
                                                outlineActionIcon(
                                                    mData.deletedAt == null
                                                    ...
                                                    Colors.red,
                                                    '${mData.deletedAt == null ? language.delete : language.force_delete}',
                                                    () {
                                                  commonConfirmationDialog(
                                                      context,
                                                      DIALOG_TYPE_DELETE, () {
                                                    if (getStringAsync(
                                                      ...
                                                      isForceDelete:
                                                          mData.deletedAt !=
                                                              null,
                                                      title:
                                                          language.delete_state,
                                                      subtitle: language
                                                          .do_you_want_to_delete_this_state);
                                                }),
                                              ],
                                            ),
                                          ),
