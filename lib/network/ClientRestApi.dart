...
Future<ClientDashboardModel> getClientDashboard() async {
  return ClientDashboardModel.fromJson(await handleResponse(
      await buildHttpResponse('client-dashboard', method: HttpMethod.GET)));
}

Future<RegionListModel> getClientCityList(
    {required int stateId, String? name}) async {
  return RegionListModel.fromJson(await handleResponse(await buildHttpResponse(
      name != null
          ? 'city-list?country_id=$stateId&name=$name&per_page=-1'
          : 'city-list?country_id=$stateId&per_page=-1',
      method: HttpMethod.GET)));
}

/// Country
Future updateCountryCity({int? stateId, int? cityId}) async {
  MultipartRequest multiPartRequest =
      await getMultiPartRequest('update-profile');
  multiPartRequest.fields['city_id'] = cityId.toString();
  multiPartRequest.fields['country_id'] = stateId.toString();

  await sendMultiPartRequest(multiPartRequest, onSuccess: (data) async {
    if (data != null) {
      LoginResponse res = LoginResponse.fromJson(data);

      await setValue(COUNTRY_ID, res.data!.stateId.validate());
...
/// get OrderList
Future<OrderListModel> getClientOrderList(
    {required int page,
    int? perPage,
    String? orderStatus,
    String? fromDate,
    String? toDate}) async {
  String endPoint =
      'order-list?client_id=${getIntAsync(USER_ID)}&city_id=${getIntAsync(CITY_ID)}&page=$page&per_page=$perPage';
