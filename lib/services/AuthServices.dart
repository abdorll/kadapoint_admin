...
class AuthServices {
  ...
  Future<void> signUpWithEmailPassword(context,
    ...
    if (userCredential.user != null) {
      ...
      userModel.stateName = userData.data!.stateName;
                  ...
                  builder: (_) {
                    return UserCitySelectScreen(onUpdate: () {
                      Navigator.pushNamedAndRemoveUntil(
                          context, DashboardScreen.route, (route) {
                        return true;
                      });
                    });
                  });
...
getCountryDetailApiCall(int stateId, context) async {
  await getCountryDetail(stateId).then((value) {
...
getCityDetailApiCall(int cityId, context) async {
  await getCityDetail(cityId).then((value) async {
    ...
    if (RegionData.fromJson(getJSONAsync(CITY_DATA)).name.validate().isNotEmpty) {
      if (getStringAsync(USER_TYPE) == CLIENT) {
        Navigator.pushNamedAndRemoveUntil(getContext, DashboardScreen.route,
            (route) {
          return true;
        });
      }
