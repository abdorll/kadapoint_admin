...
class UserCitySelectScreenState extends State<UserCitySelectScreen> {
  ...
  List<CountryData> stateData = [];
  List<RegionData> cityData = [];
  ...
  getCountryApiCall() async {
    await getCountryList().then((value) {
      ...
      selectedCountry = stateData[0].id!;
  ...
  getCityApiCall({String? name}) async {
    appStore.setLoading(true);
    await getClientCityList(stateId: selectedCountry!, name: name)
        .then((value) {
      appStore.setLoading(false);
      cityData.clear();
  ...
  Future<void> updateCountryCityApiCall() async {
    appStore.setLoading(true);
    await updateCountryCity(stateId: selectedCountry, cityId: selectedCity)
        .then((value) {
      appStore.setLoading(false);
      if (widget.isBack) {
        finish(context);
                ...
                children: [
                  Row(
                    ...
                    children: [
                      Expanded(
                          flex: 1,
                          child: Text(language.state, style: boldTextStyle())),
                      16.width,
                      Expanded(
                        flex: 2,
                        child: DropdownButtonFormField<int>(
                          ...
                          items: stateData.map<DropdownMenuItem<int>>((item) {
                  ...
                  appStore.isLoading && cityData.isEmpty
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            RegionData mData = cityData[index];
                            return ListTile(
