...
class AddCountryDialog extends StatefulWidget {
  ...
  final CountryData? stateData;
  final Function()? onUpdate;

  AddCountryDialog({this.stateData, this.onUpdate});
...
class AddCountryDialogState extends State<AddCountryDialog> {
  ...
  CountryCode? stateCode = CountryCode.fromJson(country_list.first);
  ...
  Future<void> init() async {
    ...
    if (isUpdate) {
      country_list.forEach((element) {
        if (CountryCode.fromJson(element).code == widget.stateData!.code) {
          stateCode = CountryCode.fromJson(element);
        }
      });
      if (distanceTypeList.contains(widget.stateData!.distanceType)) {
        selectedDistanceType = widget.stateData!.distanceType;
      }
      if (weightTypeList.contains(widget.stateData!.weightType)) {
        selectedWeightType = widget.stateData!.weightType;
      }
    }
  }
  ...
  AddCountryApiCall() async {
    if (_formKey.currentState!.validate()) {
      ...
      Map req = {
        "id": isUpdate ? widget.stateData!.id : "",
        "name": stateCode!.name,
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      title: Row(
        ...
        children: [
          Text(isUpdate ? language.update_state : language.add_state,
          ...
          children: [
            Form(
              ...
              child: Column(
                ...
                children: [
                  Text(language.state_name, style: primaryTextStyle()),
                  SizedBox(height: 8),
                  Container(
                    ...
                    child: CountryCodePicker(
                      initialSelection: stateCode!.code,
