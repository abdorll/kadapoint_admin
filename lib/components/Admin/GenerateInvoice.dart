...
generateInvoiceCall(OrderModel orderData) async {
  ...
  final invoice = Invoice(
    ...
    items: [
      InvoiceItem(
        product:
            '${orderData.packageType} (${orderData.totalWeight} ${appStore.stateList.isNotEmpty ? '${appStore.stateList.firstWhere((element) => element.id == orderData.stateId).weightType ?? 'kg'}' : 'kg'})',
                ...
                if (invoice.extraChargeList.length != 0)
                  Column(
                    children: [
                      ...
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: List.generate(invoice.extraChargeList.length,
                            (index) {
                          ...
                          return Padding(
                            ...
                            child: buildText(
                              title:
                                  '${item.key!.replaceAll("_", " ")} (${item.valueType == CHARGE_TYPE_PERCENTAGE ? '${item.value}%' : '${printAmount(item.value ?? 0)}'})',
...
class Utils {
  static formatPrice(double price) =>
      '${printAmount(double.parse(price.toStringAsFixed(digitAfterDecimal)))}';
